...
8. **纯函数断言**（`bin/test/run.sh`）+ **全部函数级夹具**（`bin/test/fn-*.sh`，生产 shell 设置下）。
   ⚠️ 同项内还钉了两条夹具纪律（2026-09-21）：
   ⛔ 夹具里**不得出现 `exit 0`**——唯一合法结尾是 `h_summary`；缺依赖要跳过就 **`exit 77`**，
   由循环单独计数并在「✅ 全部通过」**之前**打出「⛔ 跳过不是通过」。
   起因：`fn-fetch-events-mcp.sh` 曾用 `exit 0` 静默跳过，把 429 退避等分支藏在一片 ✅ 里（盲区⑥）
9. ShellCheck（可选）
