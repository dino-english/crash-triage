...
- **三份产物的 issue 条目一律带 8 位短 id，且 id 本身即控制台直达链接**（同 change）。
  ...
  因此把同一条 issue 判成了两条。⚠️ 链接**按载体给**（change `crash-card-issue-links` 实测订正）：卡片的 markdown 块（段一在此）**可点且不改宽度**，⛔ 表格单元格不渲染链接、写进去只会显示字面 markdown 并撑宽。⛔ 原先「只进文档不进聊天侧」是把**裸 URL** 与 **markdown 链接**混为一谈。
...
其中 36 条本文档原已覆盖，以下三条是**只在 CLAUDE.md 里有过**的，原文照录：

- ⛔ **per-issue 的「top 机型」不是结论**——实测唯一机型数≈影响安装数（一设备一机型）。
  三分支：仅 1 台设备时明说判不了、真集中才点名、其余标「分散」。
- ⛔ **性能段窗口的完整日公式是 `LCD = DATE(DATA_UNTIL)−1`**。perf 表每天只灌到
  06:59 UTC，最后一个日历日恒为 7 小时残日，DoD/WoW 两端都必须是完整日。
  ⚠️ 07:00 跑时残日还没落地，`MAX(DATE(...))` 侥幸取中完整日——**单独把 cron 挪晚
  会激活这个 bug**（F33）。
- ⚠️ **前后台归一化表达式全仓只此一处：`SQL_FG_NORM`**。取 `process_state`（双端同名
  同枚举）回落自埋 `app_foreground`；两端取值不同（Android `true/false`、iOS `1/0`）。
  ⛔ 只给绝对数——会话表无该字段，前后台没有分母。
