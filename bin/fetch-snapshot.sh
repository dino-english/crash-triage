...
FACT_CACHE_POLICY="事实层缓存（${ISSUES_DIR}/<32位id>.json，一 issue 一文件）——⛔ **只读不写**。

- ✅ **要读**：判断根因与版本流转前，先用 Read 读该 issue 的事实层文件，
  拿里面的 events 当证据。有实样本才可标「✅钻取确认」，并写清采样量。
  ⚠️ 只有读不到文件、或 events 为空时才标「⚠️聚合推断」——
  ⛔ 不得因为嫌麻烦或没去读就一律标推断。
- ⛔ **不许写**：不要创建、修改、覆盖 ${ISSUES_DIR}/ 下的任何文件，也不要为它生成脚本。
  事件明细与观测字段都由调用方在你退出后用确定性脚本写入。
  2026-09-18 实测：\`crashlytics_list_events\` 的大结果**不会下发给你**，
  你「写」出来的只会是占位条目，而那种记录会让缓存永久判命中跳过（失效模式 F52）。"
