...
# ⛔ **只有永久固定的资源才提示回填**（索引页 / 台账 / 根文件夹——它们的键不带日期）。
#    日报周报按 `daily-<日期>` / `weekly-<日期>` 键**每天/每周都会新建一份**，
#    把它们也提示出来等于天天报同一句话——实测连续 5 天每天一条，
#    噪声会把真正该回填的资源淹掉（2026-09-02 用户指出）。
# ⚠️ 判据只看键的形状，⛔ 不看标题（标题带日期与否会随文案改动漂移）。
# ⚠️ 无键的临时文档仍然提示：那是计划外新建，正该被看见。
#
# ⚠️ 2026-09-05 从 publish_doc 里抽成独立函数，**逻辑一字未改**，目的只有一个：
#    让 bin/test/fn-fixed-resource-note.sh 能用 h_load 抽**生产原文**来测。
#    ⛔ 原先夹具里手抄了一份一模一样的 case——那种测试测的是副本：本处判据改了它照样全绿，
#    与今天修的那批「判据回答不了它该回答的问题」同源。
is_fixed_resource_key() { # $1=台账键 → 0=永久固定（该提示回填） / 1=按日期新建（不提示）
  case "$1" in
