...
LEDGER_SRC="${LEDGER_SRC:-$REPOS_ROOT/dino-english-ios/reports/crash-triage/LEDGER.md}"
...
build_index() {
  local f="$ROOT/state/index-render.md"
  {
    ...
    printf '> 判断与处置结论沉淀在仓库 `reports/crash-triage/LEDGER.md`，两者冲突以仓库为准。\n'
    ...
    printf '> 装机步骤见仓库 `scripts/crash-report/INSTALL.md`；提审体检报告存 `reports/crash-triage/`，均不做在线副本以免漂移。\n\n'
    printf '## 修复状态说明\n\n| 标记 | 含义 | 判据（自动推导） |\n|---|---|---|\n'
