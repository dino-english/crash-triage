...
# ⚠️ 第 5 个入参「已存事件条数」是 2026-09-09 加的（change crash-fact-cache-events-backfill）。
#    只看计数会让「计数为正、events 数组为空」的记录在计数不再上涨后**永久判 skip**——
#    生产实测 24 个文件里 18 个卡在这个状态（计数合计 59、已存合计 6），欠账结构上补不回来。
# ⛔ 判据只取「已存 == 0 且计数 > 0」这一种**保守形态**，不做「短缺比例」：
#    events 是累积数组、计数是滚动窗口，两者本就不可比（实测开发机 110 vs 29 是正常态），
#    比例判据会把正常记录判成欠账、每轮重抓。
# ⚠️ 判定顺序不可换：强制 → 文件不存在 → 欠账 → 计数上涨 → 跳过。
# ⚠️ 缺第 5 个入参时按 -1 处理 = 「不知道」，退回旧行为——⛔ 不得默认 0，
#    那会把所有漏改的调用点变成「每轮都判欠账」，一轮放出几十次抓取。
cache_verdict() { # $1=强制重抓(1/0) $2=文件是否存在(1/0) $3=上次计数 $4=本次计数 $5=已存事件条数(可省, -1=未知) → new|append|skip
  if [ "$1" = "1" ] || [ "$2" != "1" ]; then printf 'new'; return 0; fi
  if [ "${5:--1}" -eq 0 ] 2>/dev/null && [ "$4" -gt 0 ] 2>/dev/null; then printf 'append'; return 0; fi
