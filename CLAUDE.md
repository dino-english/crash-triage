...
- ⛔⛔ **没测完、没确定，就不提交、不部署。** 有任何未验证项时，⛔ 不提交、⛔ 不提议部署、
  ⛔ 不「先上再说」、⛔ 不部署完再补「还没测」清单。未验项只有两条出路：补验，
  或**在提议之前**摆出来由人拍板。⚠️ 判据是人知情同意，不是我的置信度分数；
  ⛔「我判断它非阻塞」不算确定——2026-09-22 这么判了三次，三次全错。
- ⛔ **「测完」= 三步**：`check-scripts.sh` → 整跑 `NO_DELIVER` → **实发私聊并 `docs +fetch`
  读回产物核**。⛔ 本地全绿不构成证据：2026-09-22 在 55 条夹具 + 十项检查全绿、两次整跑
  rc=0 下实发，仍挖出三个真缺陷（F57 两处假 ✅ · F58 callout 粘连），
  并发现索引页「今日日报」入口每天都是秃的（F59）。
- ⛔ **「投递成功」≠「渲染正确」≠「内容写进去了」**：飞书写指令无权限时返回
  `ok:true`/rc=0/`result:"failed"`（F57），只判退出码会打假 ✅；排版类问题只有读回文档才看得见。
