...
- ⛔ **不设会话数门槛**（2026-08-22 起，原 `MIN_SESSIONS=5` 已废除）：门槛会把**刚开始放量或已被叫停的新版**
...
**不是对齐后的数值**。实测主力版本 0.74% / 0.79%，**首次纳入统计会直接触发红档**——这是正确行为
（问题一直存在，只是不可见），但上线前须预告，否则会被当成流水线故障排查。
...
**四个**调用点各自指定：`build_card_table()` 与 `md_table()` 用 CARD，
`md_table_doc()` 与 `xml_table()` 用 DOC，`verdict_line()` 用 DOC（结论行要评估全部指标）。
