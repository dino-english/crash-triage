...
- [x] 5.7 确认反扫只改「处置状态」与提交标识两列，不覆盖结论性备注（延后到 6.2 台账渲染实现时一并验收，见该任务） — 2026-08-20 读码确认：`scan-fix-commits.sh` 产出的 `mapped` 项只含 `{platform, commit, commit_date, subject, status}`，**无 `note` / 备注字段**，结论性备注结构上不可能被覆盖
...
- [x] 7.8 记录 L2 跑批新增耗时与 BigQuery 扫描量（待 8.3 完整 DRY RUN 时一并测量） — 2026-08-20 实测：**耗时** 纯数据版 62s（数据层含缓存命中仅 3s）／完整版 583s（差值几乎全在分析层的模型调用）；**扫描量** `crash-issues-all.sql` 双端合计 0.2 MB（`bq --dry_run` 测得，iOS 0.1 MB + Android 0.0 MB），按需定价 $6.25/TiB 折算**每次不足 $0.0001**，成本可忽略
...
- [x] 10.8 L2 真实投递到私聊 `ou_edd20a8dbfcc5e3ee279a225aec044d0`：周报文档 + 卡片 + 台账同步全部成功（合并原 8.4；首次同步走 `append` bootstrap） — 2026-08-20 实测：run `20260820-141507` EXIT=0（583s），周报文档 `KdK8dlqXuoOkQdxxz1sjszLspnc`、卡片、台账 bootstrap 三项全成
- [x] 10.9 人工抽查飞书台账：四段结构完整、现状表双端单表、时间线条目挂周报链接（合并原 8.5） — 实测四段标题齐全（项目常量 / 收口点登记 / Issue 现状表 / 变更时间线），现状表 20 行含「平台」列双端单表；本周无 issue 变化故时间线无新条目
- [x] 10.10 二次真实投递：确认现状表被 `block_replace` 更新而时间线历史完整保留（D2 核心验收点在飞书端的闭环，本地层面已由 8.6 验过） — 实测 `block_replace` 成功（block-id `doxjpCe2i4QPD0zki6FCP9zEELf`），**替换前后文档结构 diff 为空**：29 个标题的 id 与顺序一字未变，时间线历史完整保留。过程中修掉两处同源 bug（`9521c98` 标题定位、`2917a4d` 表格定位），根因是 `docs +fetch` 返回 DocxXML 文本而非块结构 JSON
- [x] 10.11 额度恢复后跑一次**带分析**的 L2，确认分析层正常产出且标注切换为「✅ 本周含深度分析」 — 实测周报由纯数据版四章扩为六章 200 行：执行完整性声明 / 本周最重要的两个结论 / 风险分级总览 / 逐项根因详情 / 修复清单 / 事实层缓存

### 10.x 真实投递中暴露、当轮修掉的问题（不在原计划内）

- [x] 10.12 `sync_ledger()` 标题定位只认 `<hN id="...">` 标题块（`9521c98`）—— keyword 检索取「第一个带 id 的对象」会命中正文里**提到**「Issue 现状表」的引用块（台账开头那段说明就写着这几个字），导致误判「标题不存在」而退回 bootstrap，实测把四段结构重复 append 了两遍。同名标题取最后一个（旧结构在上、新结构在下）
- [x] 10.13 `sync_ledger()` 表格定位改按 `<table id="...">` 标签取（`2917a4d`）—— 与 10.12 同源：`docs +fetch` 返回的是 DocxXML 文本，旧实现在 JSON 里找 `type=="table"` 永远落空。修完 10.12 才暴露出这处（原先卡在标题就退回 bootstrap 了）
- [x] 10.14 平稳周不再跳过周报文档与台账同步（`2d6c169`）—— `crash-weekly.sh` 的 quiet 分支把「内容与卡片重复」当成「不必留档」，而周报正文有卡片装不下的性能趋势与版本明细；平稳周恰恰是趋势最该留档的时候
- [x] 10.15 更正「无变化不发」这一**从未实现过的行为**（`a366a39`）—— `SEND_FLAG` 只判断 `DRY_RUN`，代码从来没判断过 `WEEK_STATE`。注释与 CLAUDE.md 描述了不存在的功能，比代码错更隐蔽：本轮就据此把 `deliver.sh` 一条死路径当成缺陷来修
- [x] 10.16 报告标题与归档带时分（`07be553`）—— 同日多次跑批覆盖同一份文档，飞书列表里只有日期分不清是哪一次的产物。L1 三处 + L2 四处标题带 `HH:MM`，归档 JSONL 新增 `at` 字段
- [x] 10.17 周报修复清单按平台分组（`fd07869`）—— 分析层产出的清单 iOS/Android 混排，读者分派任务得逐条辨认。新增 `bin/split-fix-list.py` 做投递前后处理（**只重排顺序、条目内容一字不改**，三项 diff 校验为空）；两端都提到或都没提到的进「通用 / 跨端」，不强行归类
- [x] 10.18 CLAUDE.md 补 lark-cli 勘误：`.data.document.content` 是 DocxXML 文本而非块结构 JSON（10.12/10.13 的共同根因），以及拆 `deliver.sh` 函数体复用时 `ROOT`/`STATE` 被覆盖的坑
