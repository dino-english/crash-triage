...
- [ ] 6.4 **未验**：台账 `block_replace` 稳态路径没实发走过（测试文档首轮走 bootstrap）。
      ⚠️ 该机制本 change 未改动且每周在生产运行，改的是它写入的表内容而内容渲染已验；
      要补需先给 bot 授权测试文档。
- [ ] 6.5 **未验**：带分析层的完整 L2（三次整跑都 `SKIP_ANALYSIS=1`）；
      `anr_truncated=true`（需 >50 条 ANR）；ANR 带修复提交的处置状态（今天无样本）。
