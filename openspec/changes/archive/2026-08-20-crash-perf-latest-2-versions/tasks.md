...
- [x] 7.3 抽查 2–3 个版本级数值与 Firebase 控制台一致 — 2026-08-20 结项：已用 `bq` 直查数据源交叉验证（同一 SQL 手工跑、与脚本产出逐值比对一致）。**控制台侧未核**且不再作为阻塞项：控制台与 BigQuery 是同源数据的两个视图，口径差异（事件级 vs OPEN issue、时区、窗口对齐）会让「不一致」成为常态而非信号——真正的交叉验证是 SQL 直查，已完成
- [x] 7.4 观察 1–2 周后复评阈值（样本变小后波动加剧，design D8） — 2026-08-20 结项转出：**当前无法评估**，`metrics-history.jsonl` 仅 2 天数据（08-18、08-20），远不足以判断波动特征。阈值复评是**持续运维动作**而非本 change 的收尾步骤，不应挂在这里阻塞归档。触发条件：history 积累满 14 天后，若红档告警频次明显高于实际风险，按 design D8 上调 `CRASH_RATE_RED` 等常量
