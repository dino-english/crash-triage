...
- [x] 4.5 列宽**实发验证完成**（2026-09-22，投开发机私聊 `ou_…044d0`，自测模式跳过归档/索引/台账）。
      ⛔ 此前判为「非阻塞」的依据是「8 列不是新形态」——那是推理；下面是实测：
      日报文档 https://qjphu5vphyf4.jp.larksuite.com/docx/T82udLqQJouT25xTtpbjSveBp9d
      三张明细表**每行单元格数全部是 8**，无参差无挤压；表头
      `Issue|生命周期|开关|标题|事件|影响安装|集中度|最新`；开关列三态都真实渲染
      （`fb6588bb`/`227fc097` = ✅已关闭 · `579db247` = 🔕已静音 · 其余 = 开启）；
      10 条 issue 链接末段全为 32 位 hex。

## 7. 生产核验（2026-09-23 08:30 跑批后，morning-verify）

- [x] 7.1 调度 completed（08:30:32→08:37:53）· health `ok:true` / `deliver_rc:0` · 日志 0 个 ❌
- [x] 7.2 **生产日报明细表**：12 行、每行 8 格无参差；开关列 `开启 7 · ✅已关闭 4 · 🔕已静音 1`
      ⇒ **12 行里 5 行是 Firebase 控制台默认视图看不到的**——本 change 要解决的正是这件事
- [x] 7.3 `？未取到` **0 条**：生产事实层比开发机全，补查路径工作正常
- [x] 7.4 `docs.json` 46 键 0 空值；索引页无残留占位符
