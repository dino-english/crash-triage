...
- **新建能力 `crash-perf-daily-weekly-report`**：登记崩溃 & 性能日/周报两条链路的完整契约
  - **L1 每日数据日报**（`crash-daily.sh`）：每天 07:00，BigQuery 性能 + Crashlytics 崩溃 → 飞书群卡片 + 覆盖式日报文档 + 重建索引页 + 同步台账镜像
...
**代码**：无新增/修改——`scripts/crash-report/` 已实现（`crash-daily.sh` / `crash-weekly.sh` / `fetch-snapshot.sh` / `setup.sh` / `lib.sh` / `sql/*.sql` / 两个 launchd plist），本 change 只登记契约与收编待办。
...
- 台账 `reports/crash-triage/LEDGER.md` 已登记「崩溃 & 性能日/周报」现状
...
- 台账 `reports/crash-triage/LEDGER.md` 每日同步镜像到飞书，不看仓库的人也能读
