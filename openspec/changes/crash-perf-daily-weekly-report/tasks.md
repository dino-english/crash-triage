...
- [x] 1.3 沉淀 `specs/crash-perf-daily-weekly-report/spec.md`：11 条 requirement、22 个 scenario。**2026-08-14 完成**
- [x] 1.4 沉淀 `tasks.md`（本文件）：把已实现与待办缺口收编为可跟踪清单。**2026-08-14 完成**

## 2. L1 每日数据日报（已实现）

> 实现：`scripts/crash-report/crash-daily.sh`（382 行）+ `sql/perf-{traces,screens,network}.sql` + `sessions-by-version.sql`。
...
> 实现：`scripts/crash-report/crash-weekly.sh`（203 行）+ `fetch-snapshot.sh`（full 模式）。
...
> 装机清单：`scripts/crash-report/INSTALL.md`（344 行，§0–§11）。
...
- [ ] 5.1 BigQuery `firebase_crashlytics` 出表后，把崩溃数据源从 MCP `topIssues` 换成事件级统计（消除「误关 issue 即消失」）。**待办**（开通于 2026-08-06 11:09，出表时间待控制台确认）
- [ ] 5.2 补崩溃率 SQL：分母 `firebase_sessions`（已就绪）+ 分子 `firebase_crashlytics`。**待办**（`sql/README.md` 已登记待补）
- [ ] 5.3 L2 整体超时保护：确认 `run_with_timeout` 是否覆盖全部 agent 调用，必要时引入 `gtimeout`（`brew install coreutils`）。**待办**（实测完整 triage 12 分钟以上，曾断连失败一次）
- [ ] 5.4 与 Android 侧对齐「提交信息带 issue ID」约定，打通 Android 自动修复状态判定。**待办**（2026-08-07 核实：Android 全仓 0 处 32 位 hex 引用）
- [ ] 5.5 索引页 / 台账的「固定 ID 覆盖」：lark 块 API 不支持表格（`documentBlockChildren.create` 报 `block not support to create`），v1 暂用「每次新建文档」；待调研 `documentBlockDescendant.create` 正确参数，或改「导入新文档 + 固定索引页指向最新」。**待办**
